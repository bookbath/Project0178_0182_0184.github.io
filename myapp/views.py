...
def findbus(request):
    ...
    if request.method == 'POST':
        ...
        bus_list = Room.objects.filter(building=building_r, floor=floor_r, date__year=year, date__month=month, date__day=day)
        if bus_list:
            return render(request, 'myapp/list.html', locals())
        else:
            context['data'] = request.POST
            context["error"] = "No available Room Schedule for entered Route and Date"
...
def bookings(request):
    ...
    if request.method == 'POST':
        ...
        bus = Room.objects.get(id=id_r)
        if bus:
            if bus.rem >= int(seats_r):
                name_r = bus.room_name
                cost = int(seats_r) * bus.price
                building_r = bus.building
                floor_r = bus.floor
...
def cancellings(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            bus = Room.objects.get(id=book.uid)
            rem_r = bus.rem + book.nos
            Room.objects.filter(id=book.uid).update(rem=rem_r)
